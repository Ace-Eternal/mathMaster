...
const displayQuestionNo = computed(() => props.questionNo ?? props.questionId)
...
const loadChatSessions = async () => {
  const { data } = await api.get(`/chat/questions/${props.questionId}/sessions`)
...
const loadSession = async (sessionId: number) => {
  session.value = (await api.get(`/chat/sessions/${sessionId}`, { params: { question_id: props.questionId } })).data
...
const createNewSession = () => {
  if (streamState.value !== 'idle') return
  session.value = { id: null, question_id: props.questionId, messages: [], selected_model: defaultChatModel.value }
  selectedModel.value = defaultChatModel.value
  message.value = ''
}
...
const removeSession = async (sessionId: number) => {
  ...
  try {
    ...
    await api.delete(`/chat/sessions/${sessionId}`, { params: { question_id: props.questionId } })
...
const clearAllSessions = async () => {
  ...
  try {
    ...
    const { data } = await api.delete(`/chat/questions/${props.questionId}/sessions`)
    session.value = null
    chatSessions.value = []
    createNewSession()
...
const sendMessage = async () => {
  ...
  if (!session.value) {
    session.value = { id: null, question_id: props.questionId, messages: [], selected_model: selectedModel.value }
  }
...
const initializeForQuestion = async () => {
  resetQuestionState()
  ...
  <section class="chat-shell">
    <aside class="chat-sidebar">
      ...
      <div class="chat-sidebar-summary">
        ...
        <div class="chat-sidebar-summary__value">第 {{ displayQuestionNo }} 题</div>
