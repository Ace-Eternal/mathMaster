...
- 不要输出 `answers`、`sections`、`solutions`、`boundaries`、`data`、`result` 等替代结构
- 不要输出 `question_no`、`answer_no`、`start`、`end`、`page`、`reason` 等别名字段
- 不要输出 Markdown、解释文字或代码块标记
- `items` 必须是数组；没有识别到答案时返回空数组 `[]`
- 每个答案对象必须且只能包含示例中的 8 个字段，字段名、层级和类型必须完全一致
